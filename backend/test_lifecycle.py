...
def test_order_status_progression_chain():
    ...
    # 3. Transition: AGENT_ASSIGNED -> PICKED_UP
    step1 = client.put(f"/api/orders/{order_id}/status", headers=admin_headers, json={
    ...
    # 4. Transition: PICKED_UP -> IN_TRANSIT
    step2 = client.put(f"/api/orders/{order_id}/status", headers=admin_headers, json={
    ...
    # 5. Transition: IN_TRANSIT -> OUT_FOR_DELIVERY
    step3 = client.put(f"/api/orders/{order_id}/status", headers=admin_headers, json={
    ...
    # 6. Transition: OUT_FOR_DELIVERY -> DELIVERED
    step4 = client.put(f"/api/orders/{order_id}/status", headers=admin_headers, json={
...
def test_invalid_status_transition_rejected():
    ...
    # 2. Attempt invalid illegal jump: CREATED -> DELIVERED
    bad_res = client.put(f"/api/orders/{order_id}/status", headers=admin_headers, json={
        "new_status": "DELIVERED",
        "notes": "Direct jump should fail"
    })
